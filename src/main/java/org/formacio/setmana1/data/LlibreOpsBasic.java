...
public class LlibreOpsBasic {
	...
	public Llibre carrega (String isbn) throws LlibreNoExisteixException {
		Optional<Llibre> llibre = Optional.ofNullable(em.find(Llibre.class, isbn));
		if(llibre.isPresent()) {
			return llibre.get();
		} else {
	...
	public void alta (String isbn, String autor, Integer pagines, Recomanacio recomanacio, String titol) {
		...
		nouLlibre.setPagines(pagines);
		nouLlibre.setRecomanacio(recomanacio);
		nouLlibre.setTitol(titol);
		em.persist(nouLlibre);
	}
